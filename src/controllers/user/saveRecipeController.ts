  ...
  try {
    ...
    if (user.savedRecipes.includes(recipeId)) {
      ...
      res
        .status(200)
        .send(
          `Recipe ${req.params.recipeId} has been removed from the saved recipes of user ${req.params.userId}.`
        );
      return;
    }
    ...
    res
      .status(200)
      .send(
        `Recipe ${req.params.recipeId} has been added to the saved recipes of user ${req.params.userId}.`
      );
  } catch (error) {
    console.error(error);
